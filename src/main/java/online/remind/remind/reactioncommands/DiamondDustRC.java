...
public class DiamondDustRC extends StyleRC {
	...
	protected String getDriveFormId() {
		return ModDriveFormsRM.DIAMOND_DUST.get().getRegistryName().toString();
	}

	@Override
	protected DriveForm getDriveForm() {
		return ModDriveFormsRM.DIAMOND_DUST.get();
	}
			...
			double Z = player.getZ();
				...
				for (LivingEntity target : targets){
					if (target != player){
						...
						if (p == null || (p.getMember(target.getUUID()) == null || p.getFriendlyFire())) {
							...
							target.hurt(KKDamageTypes.getElementalDamage(KKDamageTypes.ICE, player, player), damage);
				...
				for (int t = 1; t < 360; t += 20) {
					for (int s = 1; s < 360 ; s += 20) {
						...
						serverLevel.sendParticles(ParticleTypes.SNOWFLAKE, x,y,z,2,0.05,0.05,0.05,0.01);
						serverLevel.sendParticles(ParticleTypes.ITEM_SNOWBALL, x,y,z,4,0.05,0.05,0.05,0.01);

...
public class MineEntity extends ThrowableProjectile {
    ...
    public MineEntity(Level world, LivingEntity player, float damage, float dmgMult) {
        super(ModEntitiesRM.TYPE_MINE.get(), player, world);
        this.dmg = damage;
        this.blocksBuilding = true;
    }
    ...
    public void tick() {
        ...
        if (seeker && settled){
            ...
            if (closest != null) {
                Vec3 direction = new Vec3(
                        closest.getX() - this.getX(),
                        closest.getY() - this.getY(),
                        closest.getZ() - this.getZ()
                ).normalize();
        ...
        if(tickCount > 5){
            ...
            AABB triggerBox = new AABB(
                    getX() - TRIGGER_RADIUS, getY(),
                    getZ() - TRIGGER_RADIUS,
                    getX() + TRIGGER_RADIUS, getY() + 0.5,
                    getZ() + TRIGGER_RADIUS
            );
    ...
    private void settleToGround() {
        BlockHitResult hit = level().clip(new ClipContext(
                position(),
                position().add(0, -6, 0), // search downward
    ...
    private boolean canDamage(LivingEntity target) {
        ...
        Party party = WorldData.get(level().getServer())
    ...
    private void explode() {
        ...
        List<LivingEntity> targets =
                Utils.getLivingEntitiesInRadiusExcludingParty(
                        player, this,
                        DAMAGE_RADIUS, DAMAGE_RADIUS, DAMAGE_RADIUS
                );

        for (LivingEntity target : targets) {
            target.hurt(
                    KKDamageTypes.getElementalDamage(
                            KKDamageTypes.FIRE, this, getOwner()
                    ),
                    damage
            );

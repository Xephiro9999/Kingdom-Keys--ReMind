...
setTier(tier);
    ...
    public void magicUse(LivingEntity player, Player caster, float fullMPBlastMult, LivingEntity lockOnEntity) {
        ...
        switch(getTier()){
            case 0:
                dmg = playerData.getStrength(true) * (playerData.getNumberOfAbilitiesEquipped(ModAbilities.FIRE_BOOST) * 0.1f);
                break;
            case 1:
                dmg = (playerData.getStrength(true) * 1.1f) * (playerData.getNumberOfAbilitiesEquipped(ModAbilities.FIRE_BOOST) * 0.1f);
                break;
            case 2:
                dmg = (playerData.getStrength(true) * 1.25f) * (playerData.getNumberOfAbilitiesEquipped(ModAbilities.FIRE_BOOST) * 0.1f);
                break;
        }
